...
class ExtractTimezoneAbbrRefiner extends refiner_1.default {
    ...
    refine(text, results, opt) {
        results.forEach((result) => {
            ...
            const match = this.TIMEZONE_NAME_PATTERN.exec(text.substring(result.index + result.text.length));
            if (match) {
                const timezoneAbbr = match[1].toUpperCase();
                if (!this.TIMEZONE_ABBR_MAP.hasOwnProperty(timezoneAbbr)) {
                    return;
                }
                const timezoneOffset = this.TIMEZONE_ABBR_MAP[timezoneAbbr];
