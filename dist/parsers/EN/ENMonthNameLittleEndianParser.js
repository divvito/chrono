...
class ENMonthNameLittleEndianParser extends parser_1.default {
    constructor() {
        super(...arguments);
        this.PATTERN = new RegExp('(\\W|^)' +
            '(?:on\\s*?)?' +
            '(?:(Sunday|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sun|Mon|Tue|Wed|Thu|Fri|Sat)\\s*,?\\s*)?' +
            '(([0-9]{1,2})(?:st|nd|rd|th)?|' + EN_1.ORDINAL_WORDS_PATTERN + ')' +
            '(?:\\s*' +
        ...
        this.DATE_GROUP = 3;
        this.DATE_NUM_GROUP = 4;
        this.DATE_TO_GROUP = 5;
        this.DATE_TO_NUM_GROUP = 6;
        this.MONTH_NAME_GROUP = 7;
        this.YEAR_GROUP = 8;
        this.TAG = 'ENMonthNameLittleEndianParser';
    }
    pattern() {
        return this.PATTERN;
    }
    extract(text, ref, match, opt) {
        ...
        let year = EN_1.yearCalculation(match[this.YEAR_GROUP]);
        if (year) {
