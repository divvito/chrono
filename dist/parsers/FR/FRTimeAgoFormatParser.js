...
class FRTimeAgoFormatParser extends parser_1.default {
    constructor() {
        super(...arguments);
        this.PATTERN = /(\W|^)il y a\s*([0-9]+|une?)\s*(minutes?|heures?|semaines?|jours?|mois|années?|ans?)(?=(?:\W|$))/i;
        this.TAG = 'FRTimeAgoFormatParser';
        this.OTHER_PATTERNS = [
            /\w/
        ];
        this.NUM_MATCH = 2;
    ...
    extract(text, ref, match, opt) {
        ...
        const num = FR_1.matchNumber(match[this.NUM_MATCH]);
